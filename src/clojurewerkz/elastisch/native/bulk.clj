...
(defn bulk
  ...
  [operations & params]
  (let [^BulkRequestBuilder req (reduce #(add-operation %2 %1) (.prepareBulk native/*client*)
...
(defn bulk-with-index
  ...
  (bulk (map #(add-default % {:_index index}) operations) params))
...
(defn bulk-with-index-and-type
  ...
  (bulk (map #(add-default % {:_index index :_type mapping-type}) operations) params))
