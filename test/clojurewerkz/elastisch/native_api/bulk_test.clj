...
(deftest ^{:native true :indexing true} test-bulk-indexing
  (let [person fx/person-jack
    ...
    (is (= 2 (count (:items (bulk bulk-operations {:refresh true})))))
    (is (= 2 (:count (doc/count index-name index-type))))))
...
(deftest ^{:native true :indexing true} test-bulk-with-index
  (let [document fx/person-jack
        ...
        response (bulk-with-index index-name insert-operations {:refresh true})
...
(deftest ^{:native true :indexing true} test-bulk-with-index-and-type
  (let [document fx/person-jack
        ...
        response (bulk-with-index-and-type index-name index-type insert-operations {:refresh true})]
    (is (= 10 (:count (doc/count index-name index-type))))
    (is (= false (:has-failures? response)))
    (is (= 10 (count (filter #(= "create" (:op-type %)) (:items response)))))
    (is (idx/exists? index-name))))
...
(deftest ^{:native true :indexing true} test-bulk-delete
  (let [insert-ops (bulk-index (repeat 10 fx/person-jack))
        response (bulk-with-index-and-type index-name index-type insert-ops {:refresh true})
        docs (->> response :items)
        initial-count (:count (doc/count index-name index-type))
        delete-response (bulk-with-index-and-type index-name index-type (bulk-delete docs) {:refresh true})]
    (is (= 10 initial-count))
    (is (= false (:has-failures? response)))
